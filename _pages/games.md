...
Делается для удобства отправки выбранных для установки игр на консоль ик контекстного меню Windows
{: .notice--info}
...
1. Поместите в эту папку ярлык программы [dbibackend](files/dbibackend_win.zip){:target="_blank"}
    ...
    * Помните, что если вы переместите `dbibackend.exe` в другое место, то ярлык перестанет работать и его придётся создавать заново
...
1. Нажмите (START), чтобы закрыть DBI
