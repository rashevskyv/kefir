...
1. Скопируйте `keys.txt` с общими ключами и ключами TSEC, [который мы делали в процессе создания резервной копии](backup-nand#дампим-tsec-и-общие-ключи-консоли){:target="_blank"} в корень кары памяти и переименуйте его в `prod.keys`. Без этого установка `.xci`-игр работать не будет
...
1. Скопируйте скачанную игру в формате `.nsp` в папку `tinfoil/nsp` или игру в формате `.xci` в папку `tinfoil/zerotwoxci` 
...
1. Скачанные игры в формате `.nsp` должны находиться в папке `tinfoil/nsp`, игры в формате `.xci` в папке `tinfoil/zerotwoxci` 
1. Запустите "**n1dus**"
...
1. Скачанные игры в формате `.nsp` должны находиться в папке `tinfoil/nsp`
1. Запустите "**Tinfoil**"
...
1. Скачанные игры в формате `.xci` должны находиться  в папке `tinfoil/zerotwoxci` 
1. Запустите "**ZeroTwoXCI**"
1. Выберите "**Title managment**"
1. Выберите "**Install XCI**"
	* Если игр слишком много и их список не помещается в экран - программа упадёт с ошибкой. 
1. Выберите игру, которую хотите установить и нажмите (A)
1. Выберите "**Yes**" (это позволит игре запускаться на пршивках версии ниже той, для которой игра разрабатывалась)1. Выберите куда именно хотите установить игру
	* microSD storage - карта памяти 
	* System storage - внутренняя память
