...
1. Нажмите "**Выбрать .NSP файлы**" и выберите необходимые для установки `.nsp` или `.xci`
...
1. Перейдите в папку, в которой находятся игры в формате `.nsp` или `.xci`
...
{% spoiler Установка игр, DLC, обновлений с помощью Tinfoil через интернет %}

с помощью этого приложения игры можно устанавливать напрямую с удалённого сервера в интернете, который постоянно пополняется силами сообщества. По-сути программа является аналогом freeshop
...
1. Запустите **tinfoil** через [HBL](hbl){:target="_blank"}
    * Напоминаю, что HBL нужно запускать через игру, как это описано в гайде, а не через альбомы!
...
1. Перейдите в нужный раздел, выберите игру и нажмите (A)
    * В разделе **New Games** находятся все игры, которые есть на сервере      
        * Для поиска нажмите (+) на джойконе, находясь в этом разделе
    * В разделе **New Updates** находятся обновления для установленных у вас игр            
    * В разделе **New DLC** находятся DLC для установленных у вас игр             
