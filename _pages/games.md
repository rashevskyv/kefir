...
Если по какой-то причине у вас возникают проблемы при установке по USB на вашей системе, воспользуйтесь [старым методом установки с помощью NS USB Loader](games-nsul){:target="_blank"}. Этот метод доступен для всех ОС. 
{: .notice--warning}
...
1. Установите python 3.7 на ваш ПК 
    * Можете воспользоваться [этой инструкцией](https://andreyex.ru/ubuntu/kak-ustanovit-python-3-7-v-ubuntu-18-04), если не знаете как
1. Установите `pip`
...
* **В Linux**:
    ...
    1. Выполните в терминале `export PATH=$PATH:~/bin`, чтобы можно было обратиться к программе из терминала без указания пути
    1. Перейдите в папку `/home/%username%/.local/share/nautilus`
        * Пример приведён для Ubuntu с Nautilus. Если у вас другая система, команды могут отличаться
    1. Создайте там пустой файл с понятным названием, например `toSwitch` и следующим содержанием:
        <pre><code>OLDIFS=$IFS
        IFS=$'\n'
        dbibackend $NAUTILUS_SCRIPT_SELECTED_FILE_PATHS
        IFS=$OLDIFS</code></pre>
* **В MacOS**:
    У меня нет MacOS, потому я не могу составить для этой системы инструкцию. Если У вас есть мак и вы знаете как повторить подобное - [напишите мне](https://customfw.xyz/contacts). Примерный способ, который я нашёл описан здесь - [Create Context Menu Items in Mac Finder](https://davidwalsh.name/mac-context-menu)
{% endspoiler %}
