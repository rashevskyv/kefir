...
Простой способ подойдёт большинству пользователей, которые устанавливают по одной-двум играм за раз. Хоть этот способ и проще, он всё-таки менее стабильный и надёжный. В ряде случаев, при использовании простого способа, у вас могут случаться ошибки и зависания tinfoil, через который установка и будет происходить. Поэтому рекомендуется ставить игры через простой способ исключительно по одной!

{% spoiler Удобный способ (через DBI, рекомендуется) %}

{% spoiler Подготовительные работы (выполняются едино-разово) %}

{% include /inc/zadig.txt %}
1. Скачайте и установите [dbibackend installer](https://github.com/rashevskyv/switch/releases/download/418/dbibackend-setup.zip){:target="_blank"} в удобную для вас папку
	* Установщик ассоциирует dbibackend с файлами `.nsz`, `.nsp`, `.xci`
	* Установщик добавит dbibackend в меню "Отправить" в Windows
	* Если не выходит скачать инсталлер, отключите антивирус

{% endspoiler %}

{% spoiler Установка игр %}

1. Выделите файлы, которые собираетесь установить
1. В контекстном меню (вызывается по нажатию праой кнопки мыши на выделенных файлах) выберите "**Отправить**" -> "**dbibackend**"
	* Если вам нужно установить всего один файл, просто нажмите на него дважды левой кнопкой мыши
...
1. Выберите необходимые опции в "**Install options**"
	...
	* **Ignore version** - указывает на то будет ли игнорироваться параметр версии, прописанный в данных игры при установке. То есть, если игра говорит, что ей нужна прошивка 7.0, а у вас 6.2, то включение этой опции разрешит установить такую игру 
		* **Не будет работать, если игра подписана ключом версии выше, чем ваша! [Обновитесь](update-to-latest){:target="_blank"} на прошивку {% include /vars/sys_version.txt %}, чтобы не задумываться об этом**
		* Рекомендуемое значение **Yes**
1. Выберите **Start install** для начала установки
1. После окончания установки, нажмите (B) для выхода из установщика 
...
{% spoiler Простой способ (через tinfoil) %}

{% spoiler Подготовительные работы (выполняются едино-разово) %}

1. На консоли должен быть доступ к интернет! Так же убедитесь, что на приставке стоит корректная текущая дата и время 
	* Если это не так, синхронизируйте часы с интернет-временем через приложение "**nx-ntpc**" в [Homebrew Launcher](hbl){:target="_blank"}
1. Удалите "**tinfoil**" из главного меню приставки 
1. Запустите **tinfoil installer** через [Homebrew Launcher](hbl){:target="_blank"}
1. Прожмите **ОК** во всех всплывающих окнах
1. Перейдите в "**Очередь**" и проверьте есть ли там "**tinfoil[050000BADDAD0000]**", дождитесь окончания его загрузки
	* Если "**tinfoil[050000BADDAD0000]**" там нет, то проверьте время на приставке и что на приставке есть интернет 
1. Перезапустите **tinfoil** 

{% endspoiler %}

{% spoiler Установка игр %}

1. Запустите **tinfoil** из главного меню
    * К вашему ПК примонтируется новое MTP-устройство
1. Откройте появившееся устройство "**Switch**"
1. Перейдите в папку "**switch**" -> "**Установка**"
1. Перетяните и скопируйте игры, которые хотите установить в папку "**Установка**"
	* Этот способ устанавливает игры только на карту памяти!
	* Не забывайте, что игры в формате NSZ после установки занимают больше места, чем весят сам NSZ-файл 
	* Рекомендуется производить установку игр по одной, во избежании проблем и ошибок!
1. Перейдите в "**Консоль**" на экране приставки и обратите внимание на то, что там написано
	* "**disabling sleep**" - во время остановки экран приставки не будет выключаться автоматически
	* "**blocking home button**" - блокировка кнопки (HOME), чтобы пользователь не мог прервать установку
	* "**beginning stream install**" - передача игры началась
	* "**bypass firmware check**" - проверка соответствия версии системного ПО отключена
	* "**installing record %TITLEID%**" - добавление записи об игры в базу данных свитча
	* "**successfully commited metadatabase!**" - игра успешно добавлена
	* "**enabling sleep**" - включение автоматического отключения экрана
	* "**unblocking home button**" - разблокировка кнопки (HOME)
1. Только после того, как вы увидите "**unblocking home button**" в логе установки, можно закрывать tinfoil!
