...
Пользователи прошивки 5.0.1 просто обновите прошивку через настойки приставки и переходите к [следующему шагу](launch-cfw) 
...
1. Запустите [ReiNX](reinx){:target="_blank"}
	* Если вы обновляетесь с прошивки 1.0.0, то вам необходимо запускать [RajNX Debug](rajnx){:target="_blank"}
	* Если по каким-то причинам у вас не запускается кастомная прошивка, воспользуйтесь [обновлением вручную](update-to-latest_old)!
1. Откройте [Homebrew Launcher](launch-hbl){:target="_blank"}
1. Запустите ChoiDujourNX
	* Если по каким-то причинам у вас не запускается ChoiDujourNX, попробуйте запустить его из под другого кастома, если и это не помогает, воспользуйтесь [обновлением вручную](update-to-latest_old)!
1. Перейдите в папку, в которой находится прошивка 5.1.0, скопированная ранее и нажмите "**Choose**"
1. Выберите "**5.1.0 (exFAT)**"
...
Обратите внимание, что при выборе **RajNX with nogc-patch** или при запуске **ReiNX** с файлом `nogc` в папке `ReiNX` у вас перестанет работать слот картриджа на прошивке 5.1.0. Это не ошибка, это сделано намеренно, чтобы не дать прошивке слота вашего картриджа обновиться! 
