...
1. Отформатируйте карту, которую собираетесь использовать в exFAT стандартными средствами вашей ОС, если она в другой файловой системе
	...
	* Это необходимо для того, чтобы при обновлении установились так же драйвера exFAT. Если вы будете устанавливать их отдельно позже, то AutoRCM будет отключена!
...
1. Добавьте строку `kip1=kip1patch=nogc,nosigchk` в блок "[LayeredFS + Clear Log]" сразу перед строкой "atmosphere=1"	
...
Если вы захотите запустить другую конфигурацию hekate, не забудьте добавить строку `kip1=kip1patch=nogc,nosigchk` в конфиг. Помните, что запуск SX OS или любой другой прошивки без этого патча приведут к обновлению прошивки слота и при даунгрейде на прошивку ниже, чем 4.0.0 он перестанет рабоать! Это значит, что слот не будет работать ТОЛЬКО на прошивке ниже, чем 4.0.0. На любой прошивке выше он будет работать в обычном режиме! 
{: .notice--warning}		
...
Теперь при каждом запуске "LayeredFS + Clear Log" у вас будет запускаться кастомная прошивка с поддержкой LayeredFS, чистится логи и отключается проверка подписи и патчится системный модуль, позволяющий не обновлять прошивку слота для картриджей. Если вы захотите запустить другую конфигурацию hekate, не забудьте добавить строку `kip1=kip1patch=nogc,nosigchk` в конфиг. Помните, что запуск SX OS или любой другой прошивки без этого модуля приведут к обновлению прошивки слота и при даунгрейде на прошивку ниже, чем 4.0.0 он перестанет работать! Это значит, что слот не будет работать ТОЛЬКО на прошивке ниже, чем 4.0.0. На любой прошивке выше он будет работать в обычном режиме! 
{: .notice--warning}
