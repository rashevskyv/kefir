...
1. Запустите [кастомную прошивку](launch-cfw){:target="_blank"}
	* Если у вас прошивка версии 1.0.0 и вы испытываете проблемы с запуском прошивки или **ChoiDujourNX** (ниже по тексту), то вам нужно использовать другой пак для обновления. Скачайте [предыдущую версию пака](https://github.com/rashevskyv/switch/releases/tag/sdfiles_atmos){:target="_blank"}, удалите всё, что есть на карте памяти свитча, кроме папки `Nintendo` и закиньте на карту содержимое только что скачанного `sdfiles.zip`. Запускайте прошивку через hekate, выбрав **"Launch"** -> **"Atmosphere DEBUG"**. Не забудьте обновить пак на карте после того, как поднимите прошивку до **{% include /vars/sys_version.txt %}**
