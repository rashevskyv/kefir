...
1. Перейдите в меню, соответствующее прошивке с которой вы обновлялись:
	* при обновлении с прошивки ниже, чем **4.0.0** выберите **Launch firmware** -> **CFW with nogc patch**
	* при обновлении с **4.0.0** включительно и выше выберите **Launch firmware** -> **CFW**
	* Если вместо меню выбора у вас запускается логотип кастома и сразу грузится приставка, значит у вас активирован Autoboot. Нажмите (VOL-) в момент появления логотипа
1. Приставка загрузится в штатном режиме 

Теперь при каждом запуске **CFW with nogc patch** или **ReiNX with nogc patch** у вас будет запускаться кастомная прошивка, чистится логи, отключается проверка подписи и патчится системный модуль, позволяющий не обновлять прошивку слота для картриджей. Если вы захотите запустить другую конфигурацию hekate, не забудьте добавить строку `kip1=kip1patch=nogc,nosigchk` в конфиг. Помните, что запуск другой прошивки без этого модуля приведут к обновлению прошивки слота и при даунгрейде на прошивку ниже, чем 4.0.0 он перестанет работать! Это значит, что слот не будет работать ТОЛЬКО на прошивке ниже, чем 4.0.0. На любой прошивке выше он будет работать в обычном режиме! 
{: .notice--warning}

Обратите внимание, что при выборе **CFW with nogc patch** или **ReiNX with nogc patch** у вас перестанет работать слот картриджа на прошивке 5.1.0. Это не ошибка, это сделано намеренно, чтобы не дать прошивке слота вашего картриджа обновиться! 
