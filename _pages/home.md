...
header:	
  ...
  overlay_filter: 0.5
  caption:
excerpt: 'Полное руководство по прошивке Nintendo Switch<br />**Последнее изменение:** 14 сентября'
---
...
+ Вы можете купить уже прошитую приставку [у меня, если живёте в Украине](https://3dscfw.olx.ua/){:target="_blank"}, или же у ребят из списка выше
...
Предохранитель из специального массива, указывающий на версию консоли. «Сжигается» при обновлении прошивки на определённые версии, бережётся владельцами консолей со старыми версиями прошивок для возможного взлома без использования аппаратной уязвимости.
{% endspoiler %}

{% spoiler Hekate (хеката) %}
...
{% spoiler Прошивка (установка кастома) %}
...
+ После каждой перезагрузки приставки вам нужно будет запускать взлом заново. [Специальный {% include abbr/dongle.txt abbr="донгл" %}](http://3dscfw.olx.ua){:target="_blank"} или [модчип](http://3dscfw.olx.ua){:target="_blank"} существенно упростят вам жизнь (только в случае наличия аппаратной уязвимости)
