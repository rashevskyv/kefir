...
header:	
  ...
  overlay_filter: 0.5
  caption:
excerpt: 'Полное руководство по прошивке Nintendo Switch<br />**Последнее изменение:** 21 апреля'
---
...
Именно последний пункт и является нашей конечной целью. Для её достижения, если ваша приставка находится на прошивке ниже {% include /vars/last_version.txt %}, мы будем обновлять её хитрым способом - оставляя целыми предохранители в консоли. Дело в том, что при каждом обновлении прошивки Switch сжигает по одному предохранителю из специального массива, созданного именно для целей контроля версий. 
...
+ После каждой перезагрузки приставки вам нужно будет запускать взлом заново. [Специальный донгл](http://3dscfw.olx.ua){:target="_blank"} или [модчип](http://3dscfw.olx.ua){:target="_blank"} существенно упростят вам жизнь
...
+ В любой момент вы можете получить бан, даже если ваша цель всего лишь запуск Homebrew
	...
	+ Наиболее действенным методом против бана на данный момент является использование для нелицензионного ПО EmuNAND. К сожалению, он есть только на платной прошивке [SXOS](){:target="_blank"}.
