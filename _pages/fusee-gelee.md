...
* Пейлоад, который вы собираетесь запускать на приставке необходимо положить в корень карты памяти консоли (в формате `.bin`)
* Поместите [boot.dat](https://sx.xecuter.com/download/SXOS_v1.3.zip){:target="_blank"} SX OS в корень карты памяти приставки
...
1. При появлении загрузочного экрана SX OS, зажмите и удерживайте (VOL+) (замыкатель должен быть вставлен в Switch)
