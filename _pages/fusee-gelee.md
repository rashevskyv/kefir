...
* Карта памяти 
* <abbr title="Устройство, позволяющее отправить на консоль пейлоад, в качестве хоста может использоваться ПК, смартфон на Android (iOS, если есть джейлбрейк) или специальное устройство – донгл">Хост</abbr> с подготовленным пейлоадом и возможность подключить к нему Switch (например, если хост - ПК, понадобится кабель USB type-C)
...
1. Зажмите (VOL+) и вставьте ваш USB type C кабель, подключённый к <abbr title="Устройство, позволяющее отправить на консоль пейлоад, в качестве хоста может использоваться ПК, смартфон на Android (iOS, если есть джейлбрейк) или специальное устройство – донгл">хосту</abbr>, в Switch
	* Если у вас <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr> - просто вставьте его в консоль
...
В общем случае переход в RCM осуществляется одновременным зажатием (POWER) + (VOL+) на выключенной приставке при вставленном замыкателе. В этой инструкции вставленный кабель, подключенный к <abbr title="Устройство, позволяющее отправить на консоль пейлоад, в качестве хоста может использоваться ПК, смартфон на Android (iOS, если есть джейлбрейк) или специальное устройство – донгл">хосту</abbr>, включает приставку, что эквивалентно нажатию кнопки (POWER)
{: .notice--info}
...
Выберите платформу, которая будет использоваться в качестве <abbr title="Устройство, позволяющее отправить на консоль пейлоад, в качестве хоста может использоваться ПК, смартфон на Android (iOS, если есть джейлбрейк) или специальное устройство – донгл">хоста</abbr>:

* [Запуск через <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr>](#запуск-через-донгл)
...
# Запуск через <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr> 

## R4s Dongle

### Что понадобится

* Сам [<abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr>](http://vk.customfw.xyz){:target="_blank"}
* Установленная на <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгле</abbr> hekate
	* Инструкция для [R4S](r4#прошивка-донгла){:target="_blank"}
	* Инструкция для [RCMLoader](rcmloader#прошивка-донгла){:target="_blank"}

Для запуска hekate просто вставьте <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr> в приставку, когда она находится в [режиме RCM](fusee-gelee#часть-i---вход-в-rcm){:target="_blank"}. Для запуска любого другого пейлоада читайте дальше. 
...
* Пейлоад, который вы собираетесь запускать
	* Для запуска hekate она должна быть прошита в <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr>, дополнительно её никуда не нужно скидывать
* Приставка должна быть в [режиме RCM](fusee-gelee#часть-i---вход-в-rcm){:target="_blank"}

<abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">Донгл</abbr> необходимо зарядить комплектным кабелем, либо вставить во **включённую** приставку
{: .notice--warning}

Ниже описан метод запуска вообще любого пейлоада с помощью <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгла</abbr>, на котором прошита hekate. Если в вашем донгле не hekate, а SX OS, [метод запуска](fusee-gelee#sx-os-pro--r4s-dongle-с-прошитым-пейлоадом-от-sx-os) будет отличаться. Для запуска просто прошивки перейдите [сюда](launch-cfw){:target="_blank"}
...
1. Вставьте <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr> в консоль
	* Для запуска hekate <abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr> быть должен быть прошит. hekate запустится сразу, как только вы вставили донгл в консоль в режиме RCM. Если ваша цель была запуск hekate - вы её достигли, можете доставать [замыкатель](#замыкатель) 
...
* Xecuter Dongle, входящий в комплект SX PRO 
	* Либо любой другой [<abbr title="Специальное устройство небольшого размера, единственной задачей которого является отправка пейлоада на консоль.">донгл</abbr>](hhttp://vk.customfw.xyz){:target="_blank"} с [прошитым пейлоадом от SX OS]
		* Инструкция для [R4S](r4#прошивка-донгла){:target="_blank"}
		* Инструкция для [RCMLoader](rcmloader#прошивка-донгла){:target="_blank"}
* Пейлоад, который вы собираетесь запускать
...
* Пейлоад, который вы собираетесь запускать на приставке
* USB type-С на USB type-A кабель для подключения Switch к <abbr title="Устройство, позволяющее отправить на консоль пейлоад, в качестве хоста может использоваться ПК, смартфон на Android (iOS, если есть джейлбрейк) или специальное устройство – донгл">хосту</abbr>
