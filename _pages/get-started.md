...
Прежде чем начать, удалите все активные точки доступа на вашей консоли
...
Рекомендуем держать интернет отключённым, но если всё-таки очень надо, чтобы он был, пропишите DNS по инструкции ниже 
...
1. Если во время заставки не нажимать (VOL-), сразу запустится кастомная прошивка, если нажать - попадёте в hekate 
...
# Следующий шаг: [Выбор кастомной прошивки](launch-cfw){:target="_blank"}
{: .notice--success}