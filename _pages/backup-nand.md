...
Этот способ требует наличия карты памяти не менее 64х Гб. Если у вас нет таковой, но есть карта меньшего размера, бекап будет делаться по частям. После заполнения текущего объема на карте на экране будет выведена информация о том. что вам необходимо перенести папку backup на ПК, затем вернуть карту в консоль и продолжить создание резервной копии. После того, как все части бекапа будут находиться в одной папке на компьютере, их нужно склеять, используя скрипт [joiner_scripts_for_windows_linux_macos.zip](https://github.com/CTCaer/hekate/releases/download/v4.2/joiner_scripts_for_windows_linux_macos.zip){:target="_blank"}. Этот способ рассматриваться подробно не будет. Он занимает **намного** больше времени. 
{: .notice--info}
...
* Свежая версия [Zadig](http://zadig.akeo.ie/downloads/zadig.exe){:target="_blank"}
...
Дамп этих ключей подразумевает умение [запускать кастомную прошивку](launch-cfw){:target="_blank"} и [Homebrew Launcher](launch-hbl){:target="_blank"}. Вы можете пропустить сейчас этот пункт и вернуться к нему позже
{: .notice--warning}
...
1. Дождитесь окончания генерации и нажмите (+) для выхода из программы
1. Отключите приставку и поместите её карту памяти в ПК
1. Скопируйте полученные ключи `/switch/kezplez-nx/keys.txt` в папку `backup`, которая находится в корне карты памяти приставки

## Часть III - Копирование резервной копии на ПК 

1. Скопируйте папку `backup` из корня карты в надёжное место
1. По окончанию копирования, удалите папку `backup` и папку `/switch/kezplez-nx/` с карты памяти

Если вы делали резервное копирование NAND на новом свитче, в NAND которого еще не устанавливали игр, то архивирование резервной копии даст качественный результат
{: .notice--warning}
...
1. Перейдите в `Tools -> Restore` и выберите что вы хотите восстановить
	* "**Restore eMMC BOOT1/0**" - восстановление загрузочных секторов вашей консоли
	* "**Restore eMMC RAW GPP (exFAT only)**" - восстановление полной резервной копии NAND вашей приставки
	* "**Restore GPP partition**" - восстановление резервной копии NAND по частям (если вы бекапили по частям)
