...
1. Запустите пейлоад {% include inc/hekate.txt %} с помощью [Fusée Gelée](fusee-gelee){:target="_blank"} **без карты памяти**
...
Следующий шаг: [Дампим ключи](#часть-ii---дампим-boot0-1)
{: .notice--success}
...
1. Перейдите в меню "**Launch**" -> "**Payloads**"
1. Выберите "**biskeydump.bin**"
1. Когда появится смайлик, нажмите (Vol-), чтобы сохранить ключи на карту памяти 
...
1. Переместите файл `device.keys` в папку `backup` 
...
1. Перейдите в `Tools -> Restore` и выберите "**Restore eMMC BOOT0/1**"
