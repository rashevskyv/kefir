...
1. Отсканируйте появившийся на экране приставки QR-код с помощью смартфона (для Android рекомендую [эту](https://play.google.com/store/apps/details?id=com.teacapps.barcodescanner&hl=ru){:target="_blank"} программу) и отправьте на ПК полученные ключи (я отправляю себе в телеграмм, можно на почту себе отправить, или в вк, проявите фантазию)
...
1. В папке `backup` создайте текстовый файл `keys.txt` и сохраните в него ключи, что вы получили в прошлой части
