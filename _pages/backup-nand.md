...
Дамп этих ключей подразумевает умение [запускать кастомную прошивку](launch-cfw){:target="_blank"} и [Homebrew Launcher](launch-hbl){:target="_blank"}. Вы можете пропустить сейчас этот пункт и вернуться к нему позже, после того, как научитесь делать дамп
{: .notice--warning}
